...
function preProcessImageData(imgData, options) {
  ...
  var useAdaptiveThreshold = options.useAdaptiveThreshold; // Adaptive Threshold
  ...
  if (useAdaptiveThreshold) {
    ...
    for (var i = 0; i < width; i += 1) {
      ...
      for (var _j = 0; _j < height; _j += 1) {
        var index = (_j * width + i) * channels; // greyscale
        ...
        if (i === 0) {
          integralImage[_j][i] = sum;
        } else {
          integralImage[_j][i] = (integralImage[_j][i - 1] || 0) + sum;
        }
      }
    }

    console.log(data);
    console.log(integralImage); // skip edge rows

    for (var _i = 1; _i < width - 1; _i += 1) {
      for (var _j2 = 1; _j2 < height - 1; _j2 += 1) {
        var iLeft = _i - 1;
        var iRight = _i + 1;
        var jTop = _j2 - 1;
        var jBottom = _j2 + 1;
        var count = (iRight - iLeft) * (jBottom - jTop);

        var _sum = integralImage[iLeft][_j2] + integralImage[iRight][_j2] + integralImage[_i][jTop] + integralImage[_i][jBottom];

        console.log(count, _sum);
      }
    }
    ...
    for (var _i2 = 0; _i2 < data.length; _i2 += channels) {
      var r = data[_i2];
      var g = data[_i2 + 1];
      var b = data[_i2 + 2];
...
var code128 = function code128(lines) {
  ...
  switch (letterKey) {
    case '211232':
      lookupTBL = TBL_C; // sumOP = 105

      break;

    case '211412':
      lookupTBL = TBL_A; // sumOP = 103

      break;

    case '211214':
    default:
      lookupTBL = TBL_B; // sumOP = 104
  ...
  for (var i = 1; seq.length > 12; i += 1) {
    letterKey = computeGroup(seq.splice(0, 6));
    keyIndex = WIDTH_TBL.indexOf(letterKey); // sumOP += i * keyIndex
...
function combineAllPossible(results) {
  ...
  results.sort(function (a, b) {
    ...
    var length = result.length; // continue if new result is larger in size, most probable

    if (maxLength === 0 || length === maxLength) {
      maxLength = length; // update finalResult if char is feasible

      result.split('').forEach(function (char, index) {
...
function javascriptBarcodeReader(_x, _x2) {
  return _javascriptBarcodeReader.apply(this, arguments);
}

function _javascriptBarcodeReader() {
  _javascriptBarcodeReader = asyncToGenerator(
  /*#__PURE__*/
  regenerator.mark(function _callee(image, options) {
    var results, list, _ref, data, width, height, channels, sPoints, slineStep, rowsToScan, i, sPoint, start, end, processedData, lines, result;
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 7:
            ...
            slineStep = Math.floor(height / sPoints.length); //should be odd number to be able to find center

            rowsToScan = Math.min(3, height);
            i = 0;
          ...
          case 16:
            ...
            sPoint = sPoints[i]; // create section of height 3

            start = channels * width * Math.floor(slineStep * sPoint);
            end = start + rowsToScan * channels * width;
            processedData = utiltities.preProcessImageData({
              data: data.slice(start, end),
              width: width,
              height: rowsToScan,
              channels: channels
            }, options);
            lines = utiltities.getLines(processedData);
          ...
          case 40:
            return _context.abrupt("return", combineAllPossible(results));
...
var src = javascriptBarcodeReader;
