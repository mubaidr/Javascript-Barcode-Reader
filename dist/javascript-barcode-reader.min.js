(function (global, factory) {
	...
	function applyAdaptiveThreshold(data, width, height) {
	  ...
	  for (var i = 0; i < width; i += 1) {
	    ...
	    for (var j = 0; j < height; j += 1) {
	      ...
	      var index = pureIndex * channels; // greyscale
	  ...
	  for (var _i = 0; _i < width; _i += 1) {
	    for (var _j = 0; _j < height; _j += 1) {
	      var _pureIndex = _j * width + _i;

	      var _index = _pureIndex * channels;

	      var threshold = 0.15;
	      var s = height / 8; // bracket size

	      var s2 = s / 2; // no. of pixels per window
	      ...
	      var _sum = integralImage[y2 * width + x2] - integralImage[y1 * width + x2] - integralImage[y2 * width + x1] + integralImage[y1 * width + x1];

	      var _v = 255;

	      if (data[_index] < _sum * (1 - threshold) / count) {
	        _v = 0;
	      }
	...
	var utiltities = {
	  ...
	  getLines: getLines,
	  applySimpleThreshold: applySimpleThreshold,
	  applyAdaptiveThreshold: applyAdaptiveThreshold
	};
	...
	function _javascriptBarcodeReader() {
	  ...
	  regenerator.mark(function _callee(image, options) {
	    var results, list, _ref, data, width, height, channels, sPoints, slineStep, rowsToScan, i, sPoint, start, end, dataSlice, lines, result;
	        ...
	        switch (_context.prev = _context.next) {
	          ...
	          case 7:
	            ...
	            if (options.useAdaptiveThreshold) {
	              data = utiltities.applyAdaptiveThreshold(data, width, height);
	            }
	          ...
	          case 17:
	            ...
	            if (!options.useAdaptiveThreshold) {
	              dataSlice = utiltities.applySimpleThreshold(dataSlice, width, rowsToScan);
	            }

	            lines = utiltities.getLines(dataSlice, width, rowsToScan);
