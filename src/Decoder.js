let decoder = options => {
  ...
  let ScanImage = {
    data: new Uint8ClampedArray(options.data.scan),
    width: options.data.scanWidth,
    height: options.data.scanHeight,
  }
  switch (options.data.rotation) {
    case 8:
      ScanImage.data = Rotate(
        ScanImage.data,
        ScanImage.width,
        ScanImage.height,
        -90
  ...
  if (options.data.postOrientation) {
    postMessage({
      result: Image,
  ...
  let SecureCodabar = true

  let Secure2Of5 = true

  let Multiple = true
  if (typeof options.data.multiple !== 'undefined') {
    Multiple = options.data.multiple
  }
  if (typeof options.data.decodeFormats !== 'undefined') {
    decodeFormats = options.data.decodeFormats
  }
    ...
    for (y = 0; y < data.length; y += width * 4) {
      for (x = 0; x < width * 4; x += 4) {
        ...
        for (i = 1; i < 2; i++) {
          if (x + i * 4 < width * 4) {
            horizontalDiff =
              horizontalDiff + Math.abs(data[y + x] - data[y + x + i * 4])
          }
          if (y + width * 4 * i < data.length) {
            verticalDiff +=
              verticalDiff + Math.abs(data[y + x] - data[y + x + width * 4 * i])
          }
        }
  ...
  function maxLocalization(max, maxPos, data) {
    ...
    do {
      ...
      let newRect = [[minX, maxX], [minY, maxY]]
      for (i = 0; i < rects.length; i++) {
        if (Intersects(newRect, rects[i])) {
          if (rects[i][0][1] - rects[i][0][0] > newRect[0][1] - newRect[0][0]) {
            rects[i][0][0] =
              rects[i][0][0] < newRect[0][0] ? rects[i][0][0] : newRect[0][0]
            rects[i][0][1] =
              rects[i][0][1] > newRect[0][1] ? rects[i][0][1] : newRect[0][1]
            newRect = []
            break
          } else {
            rects[i][0][0] =
    ...
    } while (max > originalMax * 0.7)
    return rects
  }

  function ImgProcessing() {
    greyScale(Image.data)
    let newData = IntensityGradient(Image.data, Image.width)
    ...
    for (i = 0; i < rects.length; i++) {
      ...
      for (x = rects[i][0][0] * 2; x < rects[i][0][1] * 2; x++) {
        ...
        for (y = rects[i][1][0] * 2; y < rects[i][1][1] * 2; y++) {
          tempArray.push([
            ScanImage.table[x][y][0],
            ScanImage.table[x][y][1],
            ScanImage.table[x][y][2],
            255,
          ])
        }
    ...
    for (z = 0; z < allTables.length; z++) {
      ...
      do {
        ...
        for (i = 0; i < tempData.length; i += 4) {
          val = Math.round(
            (tempData[i] + tempData[i + 1] + tempData[i + 2]) / 3
          )
          hist[val] = hist[val] + 1
        }
        ...
        letiationData = yStraighten(tempData, start, end)
        Selection = BinaryString(letiationData)
        if (Selection.string) {
          ...
          if (format === 'EAN-13') {
            if (typeof eanStatistics[Selection] === 'undefined') {
              ...
              eanStatistics[Selection].count =
                eanStatistics[Selection].count + 1
              eanStatistics[Selection].correction =
                eanStatistics[Selection].correction + tempObj.correction
            }
            Selection = false
          }
        } else {
      ...
      if (!Selection) {
        ...
        if (format === 'EAN-13') {
          let points = {}
          for (let key in eanStatistics) {
            eanStatistics[key].correction =
              eanStatistics[key].correction / eanStatistics[key].count
  ...
  function yStraighten(img, start, end) {
    let average = 0
    let threshold
    let newImg = new Uint8ClampedArray(Image.width * (end - start + 1) * 4)
    let i, j
  ...
  function BinaryConfiguration(binaryString, type) {
    ...
    if (type === 'Code128' || type === 'Code93') {
      ...
      do {
        ...
        if (type === 'Code93' && binaryString.length < 6)
          binaryString.splice(0, totalBars)
    ...
    for (j = 0; j < img.length - Image.width * 4; j += Image.width * 4) {
      ...
      for (i = 0; i < FormatPriority.length; i++) {
        ...
        if (
          ...
          (FormatPriority[i] === 'Code39' && binaryString.length > 2)
        ) {
          if (FormatPriority[i] === 'Code128') {
            ...
            FormatPriority[i] === '2Of5' ||
            FormatPriority[i] === 'Inter2Of5'
          ) {
    ...
    for (k = 0; k < totalBinArray.length; k++) {
        ...
        if (k === 0 || k === totalBinArray.length - 1) {
          ...
          if (SecureCodabar) {
            ...
            for (i = 0; i < wideValues.length; i++) {
              if (
                wideValues[i] / wideAvrg > upperTolerance ||
                wideValues[i] / wideAvrg < lowerTolerance
              )
                return []
            }
            narrowAvrg = 0
            ...
            for (i = 0; i < BinArray.length; i++) {
              if (
                ...
                continue
              if (
                BinArray[i] / narrowAvrg > upperTolerance ||
                BinArray[i] / narrowAvrg < lowerTolerance
              )
                return []
            }
          }
          ...
          if (blackMax[0] / whiteMax[0] > 1.55) {
            ...
            if (SecureCodabar) {
              ...
              for (i = 0; i < blackMax.length; i++) {
                if (
                  blackMax[i][0] / wideAvrg > upperTolerance ||
                  blackMax[i][0] / wideAvrg < lowerTolerance
                )
                  return []
              }
              narrowAvrg = 0
            ...
            if (SecureCodabar) {
              ...
              wideAvrg /= 2
              if (blackMax[0] / wideAvrg > 1.5 || blackMax[0] / wideAvrg < 0.5)
                return []
              if (whiteMax[0] / wideAvrg > 1.5 || whiteMax[0] / wideAvrg < 0.5)
                return []
              narrowAvrg = 0
              for (i = 0; i < BinArray.length; i++) {
      ...
      if (type === 4 || type === 5) {
        ...
        if (Secure2Of5) {
          ...
          wideAvrg /= 2
          if (max[0][0] / wideAvrg > 1.3 || max[0][0] / wideAvrg < 0.7)
            return []
          if (max[1][0] / wideAvrg > 1.3 || max[1][0] / wideAvrg < 0.7)
            return []
          narrowAvrg = 0
          for (i = 0; i < BinArray.length; i++) {
          ...
          for (i = 0; i < BinArray.length; i++) {
            if (i === max[0][1] || i === max[1][1]) continue
            if (
              BinArray[i] / narrowAvrg > 1.3 ||
              BinArray[i] / narrowAvrg < 0.7
            )
              return []
          }
        }
      ...
      if (type === 2) {
        ...
        if (
          whiteMax[0] / blackMax[0][0] > 1.5 &&
          whiteMax[0] / blackMax[1][0] > 1.5
        ) {
          blackMax = [[0, 0], [0, 0]]
          for (j = 0; j < BinArray.length; j++) {
            if (j % 2 !== 0) {
        ...
        wideAvrg /= 3
        if (blackMax[0][0] / wideAvrg > 1.6 || blackMax[0][0] / wideAvrg < 0.4)
          return []
        if (blackMax[1][0] / wideAvrg > 1.6 || blackMax[1][0] / wideAvrg < 0.4)
          return []
        if (whiteMax[0] / wideAvrg > 1.6 || whiteMax[0] / wideAvrg < 0.4)
          return []
        narrowAvrg = 0
        for (i = 0; i < BinArray.length; i++) {
          if (i === blackMax[0][1] || i === blackMax[1][1] || i === whiteMax[1])
            continue
          narrowAvrg += BinArray[i]
        }
        narrowAvrg /= 6
        for (i = 0; i < BinArray.length; i++) {
          if (i === blackMax[0][1] || i === blackMax[1][1] || i === whiteMax[1])
            continue
          if (BinArray[i] / narrowAvrg > 1.6 || BinArray[i] / narrowAvrg < 0.4)
            return []
        }
        for (j = 0; j < BinArray.length; j++) {
          if (
      ...
      if (type === 3) {
        ...
        if (max[0][0] / max[1][0] >= 3) {
          ...
          wideAvrg /= 5
          if (
            max[0][0] / (wideAvrg * 3) < 0.02 ||
            max[0][0] / (wideAvrg * 3) > 3
          )
            return {
              data: [],
              correction: 0,
          ...
          if (
            max[1][0] / (wideAvrg * 2) < 0.02 ||
            max[1][0] / (wideAvrg * 2) > 3
          )
            return {
              data: [],
              correction: 0,
      ...
      while (counter < totalBars) {
        tempBin[counter] =
          tempBin[counter] > maxLength ? maxLength : tempBin[counter]
  ...
  function CheckCode128(string) {
    let checksum = string[string.length - 2].join('')
    let i
    ...
    if (startChar !== 'A' && startChar !== 'B' && startChar !== 'C')
      return false
  ...
  function DecodeCodaBar(string) {
    let result = ''
    let start = string[0].join('')
    let end = string[string.length - 1].join('')
    let i
    if (
      !(
        CodaBarEncoding[start] === 'A' ||
        CodaBarEncoding[start] === 'B' ||
        CodaBarEncoding[start] === 'C' ||
        CodaBarEncoding[start] === 'D'
      )
    )
      return false
    if (
      !(
        CodaBarEncoding[end] === 'A' ||
        CodaBarEncoding[end] === 'B' ||
        CodaBarEncoding[end] === 'C' ||
        CodaBarEncoding[end] === 'D'
      )
    )
      return false
    for (i = 1; i < string.length - 1; i++) {
      if (typeof CodaBarEncoding[string[i].join('')] === 'undefined')
        return false
  ...
  function DecodeEAN13(string) {
    ...
    if (trigger) return false
    let resultArray = []
    if (typeof EAN13Encoding.formats[decodeFormat.join('')] === 'undefined')
      return false
  ...
  function CheckCode93(string) {
    let checkOne = string[string.length - 3].join('')
    let checkTwo = string[string.length - 2].join('')
    let failSafe = true
    ...
    for (i = string.length - 4; i > 0; i--) {
      failSafe =
        typeof Code93Encoding[string[i].join('')] === 'undefined'
          ? false
          : failSafe
      if (!failSafe) break
  ...
  function CheckCode39(string) {
    ...
    if (
      typeof Code39Encoding[string[string.length - 1].join('')] === 'undefined'
    )
      return false
    if (Code39Encoding[string[string.length - 1].join('')].character !== '*')
      return false
    for (i = 1; i < string.length - 1; i++) {
  ...
  function DecodeCode128(string) {
    let set = Code128Encoding[string[0].join('')]
    let symbol
    let Code128Format = 'Code128'
    let resultString = ''
    let i
    for (i = 1; i < string.length - 2; i++) {
