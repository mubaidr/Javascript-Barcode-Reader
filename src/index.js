    ...
    return __awaiter(this, arguments, void 0, function* ({ image, barcode, barcodeType, options, }) {
        ...
        switch (barcode) {
            ...
            case BARCODE_DECODERS_1.BARCODE_DECODERS.msi:
                ...
                break;
            default:
                throw new Error(`Invalid barcode specified. Available decoders: ${Object.values(BARCODE_DECODERS_1.BARCODE_DECODERS).join(', ')}.`);
        }
        const useSinglePass = isTestEnv || (options && options.singlePass) || false;
        const { data, width, height } = isImageLike(image) ? image : yield (0, getImageDataFromSource_1.getImageDataFromSource)(image);
